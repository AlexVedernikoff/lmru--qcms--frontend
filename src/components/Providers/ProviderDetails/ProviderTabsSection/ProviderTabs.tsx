...
const ProviderTabs: React.FC = () => {
    ...
    return (
        <Grid className={styles.sectionItem} columns="1fr" rowGap={24} columnGap={24} >
            <TabList active={tab} style={{ color: '#999999'}} size='l' onChangeTab={function noRefCheck(_, tab){setTab(tab)}}>
                <Tab>
                    {t('ProviderDetails.ProviderTabs.ProjectCommand.Title')}
                </Tab>
                <Tab>
                    {t('ProviderDetails.ProviderTabs.Contacts.Title')}
                </Tab>
                <Tab>    
                    {t('ProviderDetails.ProviderTabs.Products.Title')} 
                </Tab>
            </TabList>
            {tab === 0 && <CommandTable />}
            {tab === 1 && <ContactsTable />}
